...
levels(dataset_train$CreditLimitCategory) <- make.names(levels(factor(dataset_train$CreditLimitCategory)))
levels(dataset_test$CreditLimitCategory) <- make.names(levels(factor(dataset_test$CreditLimitCategory)))
...
model <- train(CreditLimitCategory ~ SalesCurrentYr + SalesLastYr + DesignBandSalesLast12Mo + NumberOfStoreLocations + AvgDaysOfPayCategory + ReturnedPaymentCount + RuralUrbanContinuumCode + MetroIndicator + MedianEarnings + MedianIncomeHouseholds, data=dataset_test, method="multinom", metric="ROC", trControl=control, maxit=600)
