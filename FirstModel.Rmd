...
We chose to go with the ProjectTemplate package and structured our project accorgingly. With this setup, our data gets automatically loaded from the data directory and using the munge feature we also automatically build a **training set** and a **test set** by dividing the original dataset using a 70/30 ratio. To make sure that both the training and the test sets had a similar distribution, we have used the `sample.split()` function to have similar distributions over the **CreditLimit** variable, which we will perform predictions on.
...
We developed our own functions to be able to calculate the errors for our models. These functions include **R-squared**, **Adjusted R-squared** and the **Root Mean Square Error (RMSE)**. We will make use of these functions for our initial model and the future models to track fitness of our models and the performance of our models comared to each other.
...
We first wanted to find out if Sales would be a good indicator to predict Credit Limits for our customers. We chose the Last Year's Sales (SalesLastYr) column and build a linear regression model with one independent variable. Our assumption is whether we can formulate the Credit Limit with a simple formula as the following:
...
R-squared paramater indicats the the fitness of a model and we want this value to be closer to 1, aka we want the ratio of the sum of the square of the residuals ($SS_r$) over the sum of the square of the averages ($SS_t$) to be small so that $R^2$ becomes larger.
...
The three of the indicators that we are planning on using for this model and the next models that we are planning on building are; $R^2$, Adjusted $R^2$ and the RMSE and here is a recap of these three indicators for our training model:
...
And below are the results of the same three indicators for our test set:
...
We have performed other exploratory analysis on our data and we began our examination with the **Sales** numbers for the **Last Calender Year**. We looked at the min, max, std dev and the quintiles to understand the data better as well as to be able to do a comparison between the training and the test sets.
...
To understand the correlation between these two variables and how our model repsresents this data, we have plotted the **Credit Limit vs. Last Year's Sales** and also plotted a line representing our model.
...
It is possible that the Credit Limit cannot be explained by one varaible and a linear relationship might still exist using other independent variables. It is also possible that the relationships are non-linear and we might have to build non-linear models to explain the data.

## Next Step

We will try to improve our linear model by examining all other variables that we have in our dataset and use a **Backward Elimitaion** to find out the most significant variables which might help us to build a better model. As we add more variables to describe our model we will also utilize the Adjusted $R^2$ predictor to account for the impact of the added variables tot he $R^2$ value.

Future steps might also include adding non-linear terms to the model or building other non-linear models such as **Decision Tree** or take advantage of ensemble learning by building a model usning **Random Forest**.
